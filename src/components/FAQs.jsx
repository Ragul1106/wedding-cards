...
export default function FAQSection() {
  return (
    <section className="bg-[#E6E6FA] py-12 px-4 sm:px-10 md:px-20">
      <h2 className="text-xl sm:text-2xl font-semibold mb-10">FREQUENTLY ASKED QUESTIONS</h2>
      <div className="space-y-10">
        {faqs.map((faq, index) => (
          <div key={index}>
            <div className="bg-[#E09800] text-black text-center font-bold px-4 py-2">
              {faq.question}
            </div>
            <p className="mt-4 text-[#1a1a1a] text-2xl text-semibold whitespace-pre-line">{faq.answer}</p>
          </div>
        ))}
      </div>
    </section>
